...
import { parseReasoningPayload, ReasoningPayload } from "@/core/reasoning";
...
const ChatSingleMessage: React.FC<ChatSingleMessageProps> = ({
  ...
  // Reasoning payload parsed from CORTEX_REASONING marker
  const [reasoningPayload, setReasoningPayload] = useState<ReasoningPayload | null>(null);
  ...
  const preprocess = useCallback(
    (content: string): string => {
      ...
      // Note: processThinkSection is intentionally not called here — ThinkBlockStreamer
      // now emits CORTEX_REASONING markers instead of <think> tags, so this path is
      // dead for new messages. Old <think> tags will render as plain text (hard cut).
    ...
    <div className="tw-my-1 tw-flex tw-w-full tw-flex-col">
      ...
      >
        <div className="tw-flex tw-max-w-full tw-flex-col tw-gap-2 tw-overflow-hidden">
          ...
          {/* Reasoning panel rendered from CORTEX_REASONING marker (if present) */}
          {reasoningPayload && message.sender !== USER_SENDER && (
            <ReasoningPanel payload={reasoningPayload} />
          )}
